...
class GeminiAPIClient {
  ...
  constructor() {
    ...
    if (!apiKey) {
      console.error(
        "❌ Google Gemini API key is not set. AI suggestions will be disabled."
      );
      return;
    }
  ...
  async generateSuggestions(
    ...
    if (!this.model) {
      console.warn(
        "⚠️ Gemini API not initialized. Returning default suggestions."
      );
...
`;
    ...
    > = {
      scope: (ctx) => `
${baseInstructions}
Generate strategic risk assessment suggestions for ${
        ctx.scopeLevel || "multi-level"
      } geographic contexts.
...
`,
      context: (ctx) => `
${baseInstructions}
Develop comprehensive risk context for ${
        ctx.riskType || "multi-hazard"
      } scenarios.
Analyze stakeholder perspectives, critical infrastructure (${
        ctx.criticalAssets?.join(", ") || "various assets"
      }), 
and potential cascading effects.
`,
      identification: (ctx) => `
${baseInstructions}
Identify emerging and potential disaster risks in ${
        ctx.geographicContext || "complex environments"
      }.
Focus on:
...
`,
    ...
    return (
      sectionPrompts[section]?.(context) ||
      `
${baseInstructions}
